...
class MenuBar(Base):
    ...
    menu_toextend_loc = (By.XPATH, './/li[@class="with-children closed"]/a')
    # The locator of the page to open
    page_toopen_loc = (By.XPATH, './/li[@class="without-children closed"]/a')

    # Click the submenu or page under the current opened menu
    def action_expand_menu(self, menuName, whetherMenu = True):
        '''
        ...
        if whetherMenu:
            menu_items_loc = self.menu_toextend_loc
        else:
            menu_items_loc = self.page_toopen_loc
